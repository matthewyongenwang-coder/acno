...
def section_headline(runs: list[dict], yolo: list[dict]) -> list[str]:
    lines = [
        ...
        "",
        "> **Read acne_type's number with care.** A *frozen* ImageNet backbone, fine-tuned",
        "> on nothing, scores **73.5%** on this dataset's strict split against a 26.8%",
        "> majority baseline, and the same probe on skin_type scores 32.5% against a",
        "> 37.7% baseline, i.e. no signal at all. Blanking out the middle half of each",
        "> image, where the lesion has to be, still leaves the probe at 63.9%: nearly",
        "> 80% of its advantage survives deleting the thing it is meant to classify.",
        "> At 16x16, with no lesion detail left at all, it still scores 56.1%. So the",
        "> class is largely predictable from colour and background outside the lesion,",
        "> which the strict split cannot remove because it is shared across a whole",
        "> class rather than tied to duplicate images. Treat acne_type's headline",
        "> accuracy as an upper bound on a contaminated benchmark, not as field",
        "> performance. See DATA_QUALITY.md. Reproduce with:",
        ">",
